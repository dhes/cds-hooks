  ...
  for (const fileName of fs.readdirSync(pathToFolder)) {
    ...
    if (lib && lib.id) {
      ...
      if (!elm) {
        ...
        // console.log(json.prefetch) //DH
        // DH Create a prefetch from the cql code if one is not present in the hook
        if (!json.prefetch) {
...
function buildPrefetchQuery(retrieve) {
  ...
  if (match) {
    ...
    case 'Provenance':
    case 'QuestionnaireResponse': //DH
    case 'Questionnaire': //DH
